...
document.addEventListener('DOMContentLoaded', async () => {
    // تهيئة Supabase
    const { initializeDatabase } = window.supabaseConfig;
    const isConnected = await initializeDatabase();
    
    // إخفاء شاشة التحميل بعد 2 ثانية أو بعد الاتصال
    setTimeout(() => {
        document.getElementById('loadingScreen').classList.add('hidden');
        
        // إذا لم يكن هناك اتصال، عرض تحذير
        if (!isConnected) {
            window.supabaseConfig.showMessage(
                '⚠️ الموقع يعمل ولكن قاعدة البيانات غير جاهزة. تأكد من إنشاء الجداول في Supabase',
                'warning'
            );
        }
    }, 2000);
    ...
    // تحميل البيانات إذا كان هناك اتصال
    if (isConnected) {
...
async function loadInitialData() {
    try {
        ...
        window.supabaseConfig.showMessage('تم تحميل البيانات بنجاح', 'success');
...
function loadDemoData() {
    window.appData.allItems.mods = [
        ...
        },
        {
            id: 2,
            title: 'مود السيارات',
            description: 'أضف سيارات ومركبات مختلفة إلى عالم ماينكرافت',
...
function renderItems(items, containerId) {
    ...
    items.forEach((item, index) => {
        const isMod = item.type === 'mod' || containerId.includes('mod') || containerId === 'latestMods';
        ...
        itemElement.innerHTML = `
            ...
            <div class="item-content">
                ...
                <p class="item-desc">${item.description.substring(0, 150)}${item.description.length > 150 ? '...' : ''}</p>
                <div class="item-meta">
    ...
    container.querySelectorAll('.btn-view').forEach(btn => {
        btn.addEventListener('click', (e) => {
            const id = e.target.getAttribute('data-id') || e.target.closest('.btn-view').getAttribute('data-id');
            const type = e.target.getAttribute('data-type') || e.target.closest('.btn-view').getAttribute('data-type');
    ...
    container.querySelectorAll('.btn-download').forEach(btn => {
        btn.addEventListener('click', (e) => {
            const url = e.target.getAttribute('data-url') || e.target.closest('.btn-download').getAttribute('data-url');
...
async function increaseDownloadCount(id, type) {
    try {
        ...
        if (itemIndex !== -1) {
            itemsArray[itemIndex].downloads = (itemsArray[itemIndex].downloads || 0) + 1;
            updateStats();
        }
...
async function showItemDetails(id, type) {
    try {
        ...
        // إذا لم تجد محلياً، جلب من قاعدة البيانات
        if (!item) {
            const { supabase, TABLES } = window.supabaseConfig;
            const table = type === 'mod' ? TABLES.MODS : TABLES.VERSIONS;
        ...
        resultsContainer.innerHTML = `
            ...
            <div class="search-items">
                ${results.slice(0, 5).map(item => {
                    const isMod = item.category || !item.version;
                    return `
...
function renderAdminItems(items, containerId, type) {
    ...
    container.innerHTML = items.map(item => `
        <div class="post-item" data-id="${item.id}" data-type="${type}">
            ...
            <p>${item.description.substring(0, 100)}...</p>
            <div class="post-meta">
                <small>تاريخ الإضافة: ${formatDate(item.created_at)}</small>
